...
public class ICD
{
	...
	private int female_Height_min;
	private int female_Height_max;
	
	private int male_Height_min;
	private int male_Height_max;
	
	private int female_Weight_min;
	private int female_Weight_max;
	
	private int male_Weight_min;
	private int male_Weight_max;
	...
	public ICD(String fileName) throws JAXBException
	{
		JAXB jaxb = new JAXB();
		jaxb.read(fileName);
	...
	{
		Random r = new Random();
		int female_Height = r.nextInt(female_Height_max - female_Height_min) + female_Height_min
		        + 1;
	...
	{
		Random r = new Random();
		int female_Weight = r.nextInt(female_Weight_max - female_Weight_min + 1)
		        + female_Weight_min;
	...
	public String blood_Pressure(int top, int bot)
	{
		...
		if (top <= normal_Blood_Top & bot <= normal_Blood_Bot)
			...
			if (top <= pre_Blood_Top & bot <= pre_Blood_Bot)
				bp = "Pre";
			else
				if (top <= s1_Blood_Top & bot <= s1_Blood_Bot)
					bp = "S1";
				else
					bp = "S2";
		return bp;
	...
	public void ICD_Common(Sheet common, Sheet rare, List<String> line)
	{
		Random r = new Random();
	...
	public void ICD_Gender(String gender, Sheet male, Sheet female, List<String> line)
	{
		Random r = new Random();
		...
		else
		{
			for (int j = 0; j < randomCodes; j++)
			{
				...
				line.add(female_ICD);
				line.add(female_description);
			}
		}
		
	}
	
	public void ICD_Overweight(double BMI, Sheet overweight, List<String> line)
	{
		Random r = new Random();
	...
	public void ICD_Age(int a, Sheet age, List<String> line)
	{
		Random r = new Random();
	...
	public void ICD_Preg(int age, String gender, Sheet preg, List<String> line)
	{
		Random r = new Random();
	...
	public void ICD_BP(String bp, Sheet BP, List<String> line)
	{
		Random r = new Random();
	...
	public boolean getDrug(String gender, String race)
	{
		...
		if (gender == "Male")
		{
			if (race == "White" & chance(9))
				drug_Abuse = true;
			else
				if (race == "Black" & chance(11))
					drug_Abuse = true;
				else
					if (race == "Asian" & chance(3))
						drug_Abuse = true;
					else
						if (race == "Latino" & chance(6))
							drug_Abuse = true;
						else
							if (race == "Other" & chance(5))
								drug_Abuse = true;
		}
		if (gender == "Female")
		{
			if (race == "White" & chance(4))
				drug_Abuse = true;
			else
				if (race == "Black" & chance(6))
					drug_Abuse = true;
				else
					if (race == "Asian" & chance(1))
						drug_Abuse = true;
					else
						if (race == "Latino" & chance(3))
							drug_Abuse = true;
						else
							if (race == "Other" & chance(2))
								drug_Abuse = true;
		}
		return drug_Abuse;
	...
	public void ICD_Drug(boolean drug_Abuse, Sheet drug, List<String> line)
	{
		Random r = new Random();
